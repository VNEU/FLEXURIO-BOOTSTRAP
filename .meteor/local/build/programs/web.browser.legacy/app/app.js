...
Template.login.events({
  ...
  'submit form.login-server': function (e, tpl) {
    e.preventDefault();
    var textEmail = tpl.$('input[name=username]').val();
    var textPassword = tpl.$('input[name=password]').val();
...
Template.menu.helpers({
  ...
  menus: function () {
    var textSearch = '';
    var menuGroups = Session.get('groupMENU');
...
insertMENU = function (tpl) {
  var namaMENU = tpl.$('input[name="namaMENU"]').val();
  var routerMENU = tpl.$('input[name="routerMENU"]').val();
  var groupMENU = Session.get('groupMENU');
  var iconMENU = tpl.$('input[name="iconMENU"]').val();
...
updateMENU = function (tpl) {
  var namaEditMENU = tpl.$('input[name="namaEditMENU"]').val();
  var routerEditMENU = tpl.$('input[name="routerEditMENU"]').val();
  var iconEditMENU = tpl.$('input[name="iconEditMENU"]').val();
  var groupEditMENU = Session.get('groupMENU');
...
Template.member.events({
  ...
  'click a.saveUpload': function (e, tpl) {
    ...
    readFile(file, function (content) {
      var dataCSV = JSON.parse(csv2json(content));
      var beda = cekKolom(getKeys(MEMBER.findOne()), getKeys(dataCSV[0]));
...
cekKolom = function (oArrayOne, csvJson_One) {
  var data = $(oArrayOne).not(csvJson_One).get();
  return data;
};
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Template.profile.helpers({
  ...
  isCreatingPenyelenggara: function () {
    var penyelenggara = PenyelenggaraAdmins.findOne({
...
Template.profile.events({
  ...
  'click a.simpanPhoto': function (e, tpl) {
    var lokasi = sURL_upUser + ".upUser/pictures/memb_" + Meteor.userId() + "/member_" + Meteor.userId() + ".jpg";
...
Template.profilebar.events({
  ...
  "submit form.form-quotes": function (e, tpl) {
    e.preventDefault();
    var textQuotes = tpl.$('input[name="quotes"]').val();
    var dKaryawan = MEMBER.findOne({
      _id: Meteor.userId()
    });
    var idKaryawan = "";
...
insertWOSUBTIPEDETAIL = function (tpl) {
  var namaWOSUBTIPEDETAIL = tpl.$('input[name="namaWOSUBTIPEDETAIL"]').val();
...
updateWOSUBTIPEDETAIL = function (tpl) {
  var namaEditWOSUBTIPEDETAIL = tpl.$('input[name="namaEditWOSUBTIPEDETAIL"]').val();
...
},"styles":{"js":{"controlmenu.js":function(require){
...
+function ($) {
  ...
  $.fn.controlmenu = function (option) {
    return this.each(function () {
      ...
      var options = $.extend({}, ControlMenu.DEFAULTS, $this.data(), (0, _typeof2.default)(options) == 'object' && option);
...
+function ($) {
  ...
  $.fn.sidebar = function (option) {
    return this.each(function () {
      ...
      var options = $.extend({}, Sidebar.DEFAULTS, $this.data(), (0, _typeof2.default)(options) == 'object' && option);
      if (!data && options.toggle && option == 'show') option = !option;
...
adaDATA = function (obj) {
  try {
    ...
    if ((0, _typeof2.default)(obj) !== "object") return false; // Otherwise, does it have any properties of its own?
